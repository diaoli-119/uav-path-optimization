...
global n_obs obs obs_rad uav_finite_size uav_ws delta_t t xf;
global max_speed min_speed Md D_eta_opt;
...
if make_plot == 1
    ...
    for m = 1 : n_obs
        ...
        plot(obs(m,1),obs(m,2),'xk'); % static obstacles' centers
        x = obs(m,1) - obs_rad(m) : cs : obs(m,1)+ obs_rad(m);
        y =  (obs_rad(m)^2 - (x - obs(m,1)).^2).^0.5 + obs(m,2); %top part of circle
        y1 = -(obs_rad(m)^2 - (x - obs(m,1)).^2).^0.5 + obs(m,2); %bottom part of circle
...
if make_plot == 0
    ...
    l_l = [];
    td = 0;
    if mod(length(Bez_points),2) == 0
        l_l = zeros(length(Bez_points)/2,1);
    else
        l_l = zeros((length(Bez_points)+1)/2,1);
    end
    x_sp = [0,0];
    xi = Bez_points;
    ...
    for i = 1 : length(Bez_points)/2
        ...
        if i == 1
            ...
            for n = 1 : length(t)
                %calculate position
                p = (1-t(n))^2*x_sp(1,:) + 2*(1-t(n))*t(n)*xi(1,:)+t(n)^2*xi(2,:);
            ...
            for p = 1 : length(t)
                %calculate position
                p = (1-t(p))^2*xi(2*i-2,:) + 2*(1-t(p))*t(p)*xi(2*i-1,:)+t(p)^2*xi(2*i,:);
